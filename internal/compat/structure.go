...
var moduleHints = map[string]string{
	...
	"docker_container.*": "halite spells it container.running / container.stopped / container.absent, and drives docker or podman",
	"docker_image.*":     "halite spells it container.image_present / container.image_absent",
...
var behaviourArgs = map[string]string{
	...
	"pkg.installed:fromrepo":        "installing from one named repository is not implemented; define the repository with pkgrepo.managed",
	...
	"file.directory:recurse":        "file.directory manages one directory; use file.recurse to manage a tree",
...
var knownArgs = map[string][]string{
	...
	"file.absent":      {"name"},
	"file.recurse":     {"name", "source", "file_mode", "dir_mode", "user", "group", "template", "clean"},
	"ssh_auth.present": {"name", "user", "enc", "comment", "options", "config"},
	"ssh_auth.absent":  {"name", "user", "config"},
	"pkgrepo.managed": {"name", "url", "line", "dist", "comps", "arch", "signed_by", "source",
		"humanname", "baseurl", "metalink", "mirrorlist", "gpgkey", "gpgcheck", "priority",
		"module_hotfixes", "enabled", "refresh", "mirror_type", "signature_type", "fingerprints"},
	"pkgrepo.absent":     {"name", "refresh"},
	"pkg.installed":      {"name", "pkgs", "version", "hold"},
	"pkg.removed":        {"name", "pkgs"},
	"pip.installed":      {"name", "pkgs", "bin_env", "requirements", "upgrade"},
	"pip.removed":        {"name", "pkgs", "bin_env"},
	"virtualenv.managed": {"name", "python", "requirements"},
	"service.running":    {"name", "enable"},
	"service.dead":       {"name"},
	"service.enabled":    {"name"},
	"service.disabled":   {"name"},
	"network.system":     {"name", "hostname"},
	...
	"jail.present": {"name", "path", "hostname", "ip4_addr", "ip6_addr", "interface",
		"params", "config", "boot"},
	"jail.absent":             {"name", "config", "boot"},
	"jail.running":            {"name"},
	"jail.stopped":            {"name"},
	"container.image_present": {"name", "force", "runtime"},
	"container.image_absent":  {"name", "runtime"},
	"container.running": {"name", "image", "command", "env", "labels", "ports", "volumes",
		"network", "restart", "user", "workdir", "run_args", "runtime"},
	"container.stopped":      {"name", "runtime"},
	"container.absent":       {"name", "runtime"},
	"zfs.filesystem_present": {"name", "properties", "parents"},
	"zfs.filesystem_absent":  {"name", "recursive"},
	"zfs.snapshot_present":   {"name", "recursive"},
	"zfs.snapshot_absent":    {"name", "recursive"},
	"file.symlink":           {"name", "target", "force", "makedirs", "user", "group"},
	"file.copy": {"name", "source", "force", "makedirs", "preserve", "mode", "user", "group",
		"show_diff"},
	"file.append":    {"name", "text", "makedirs", "mode", "user", "group", "show_diff"},
	"file.prepend":   {"name", "text", "makedirs", "mode", "user", "group", "show_diff"},
	"file.comment":   {"name", "regex", "char", "mode", "user", "group", "show_diff"},
	"file.uncomment": {"name", "regex", "char", "mode", "user", "group", "show_diff"},
	"file.line": {"name", "content", "match", "mode", "location", "before", "after", "create",
		"user", "group", "show_diff"},
	"file.replace": {"name", "pattern", "repl", "count", "append_if_not_found",
		"prepend_if_not_found", "not_found_content", "ignore_if_missing", "mode", "user", "group",
		"show_diff"},
	"file.blockreplace": {"name", "marker_start", "marker_end", "content", "source", "template",
		"append_if_not_found", "prepend_if_not_found", "mode", "user", "group", "show_diff"},
	"host.present":             {"name", "ip", "names", "clean", "config"},
	"host.absent":              {"name", "ip", "names", "config"},
	"kmod.present":             {"name", "persist", "config"},
	"kmod.absent":              {"name", "persist", "config"},
	"timezone.system":          {"name"},
	"locale.system":            {"name", "key"},
	"selinux.mode":             {"name"},
	"selinux.boolean":          {"name", "value", "persist"},
	"alternatives.install":     {"name", "link", "path", "priority"},
	"alternatives.remove":      {"name", "path"},
	"alternatives.set":         {"name", "path"},
	"x509.private_key_managed": {"name", "algo", "bits", "new", "mode", "user", "group", "makedirs"},
	"x509.certificate_managed": {"name", "private_key", "CN", "cn", "O", "OU", "C",
		"subject_alt_names", "days_valid", "days_remaining", "ca",
		"signing_private_key", "signing_cert", "mode", "user", "group", "makedirs"},
