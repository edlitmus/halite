...
| macOS | 1 | `mac_brew_pkg` and `mac_service` are aliases; `mac_defaults`, `mac_power`, `mac_user`, `mac_group`, `mac_shadow`, `mac_softwareupdate` and `mac_keychain` are modules (DIVERGENCE 5.41-5.46). Six of the seven are now `hardware`, driven by hand on a real Mac under `sudo` (5.114-5.118); `mac_softwareupdate` is the one still `assumed`, for want of running `--download`. `mac_assistive` was the eighth and is out of the build — SIP keeps its writes unreachable without a grant re-given on every rebuild (5.119). |
