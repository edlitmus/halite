...
**Status: v0.12.0 — P1–P5 complete, P6 (module breadth) in progress.** Three ways to run:
...
* **Masterless** — `halite apply` on the host. Highstate with top.sls
  ...
  its `_in` forms and `names:` expansion, and 68 state functions across
  file/pkg/pkgrepo/pip/service/cmd/user/group/ssh_auth/host/jail/container/
  zfs/cron/sysctl/kmod/selinux/timezone/x509/archive/git/mount.
...
line — see [docs/service.md](docs/service.md) for those, the logging they
do, the FreeBSD rc.d scripts, and the systemd units.
...
* [docs/service.md](docs/service.md) — config files, logging, FreeBSD rc.d, systemd units
